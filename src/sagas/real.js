...
import * as api from 'services/real';
import { push } from 'react-router-redux';
import * as actions from 'actions/real';
import { getUser, getRepo, getStarredByUser, getStargazersByRepo, getRouterParams } from 'reducers/real/selectors';

// each entity defines 3 creators { request, success, failure }
const {
  user, repo, starred, stargazers,
} = actions;
...
function* loadUserPage () {
  const { login } = yield select(getRouterParams);
  yield put(actions.loadUserPage(login));
}
...
function* watchNavigate () {
  while (true) {
    ...
    yield put(push(`/real${pathname}`));
  }
}

function* watchRouter () {
  while (true) {
    yield take(actions.UPDATE_ROUTER_STATE);
    console.log('router');
    yield fork(loadUserPage);
  }
}
