import * as ActionTypes from 'actions/real';
...
function entities (state = { users: {}, repos: {} }, action) {
  if (action.response && action.response.entities) {
    return {
      users: { ...state.users, ...action.response.entities.users },
      repos: { ...state.repos, ...action.response.entities.repos },
    };
  }
  return state;
}
...
function router (state = { pathname: '/real', params: {} }, action) {
  switch (action.type) {
